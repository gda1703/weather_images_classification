...
def GetImagesFromFolder(PATH,Class_Folder,ext):
    ...
    -----------
        ...
        ext: SExtension de fichier des fichiers image à charger.
...
# Sauvegarder les images des différents dataset dans le bon répertoire en local 
...
# Fonction qui calcule et affiche la matrice de confusion
def confusion_matrix(test_labels, predicted_labels, labels):
    """
    Cette fonction permet de calculer et d'afficher la matrice de confusion
    
    Args :
    -----------
        - test_labels : les labels du jeu de données de test one-hot encoded
        - predicted_labels : les labels qui ont été prédits
        - class_names : la liste des classes du modèle
    ...
    confusion_matrix_lanet = np.zeros((num_classes, num_classes), dtype=int)
    for true_label, predicted_label in zip(test_labels_encoded, predicted_labels):
        confusion_matrix_lanet[true_label, predicted_label] += 1
        
    sns.heatmap(confusion_matrix_lanet, annot=True, cmap='Blues', fmt='d', xticklabels=labels, yticklabels=labels)
